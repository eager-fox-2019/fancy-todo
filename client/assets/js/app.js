const BASE_URL = 'http://35.247.175.185'
...
function onSignIn(googleUser) {
    ...
    $.ajax({
        url: `${BASE_URL}/api/users/googlesignin`,
        method: `POST`,
        headers: {
            token
        }
...
function removeMember(userId) {
    $.ajax({
        url: `${BASE_URL}/api/projects/${userId}/removeMember`,
        method: `PATCH`,
        headers: {
            token: localStorage.token,
            projectId: localStorage.projectId
...
function fetchProjectMember() {
    $.ajax({
        url: `${BASE_URL}/api/projects/members`,
        method: `GET`,
        headers: {
            token: localStorage.token,
            projectId: localStorage.projectId
...
function getCurrentWeather(position) {
    ...
    $.ajax({
        url: `${BASE_URL}/api/opens/currentweather`,
        method: `POST`,
        data: {
            lat,
            lng
...
$(document).ready(function() {
    ...
    $("#searchTodo").submit(function(e) {
        ...
        $.ajax({
            url: `${BASE_URL}/api/todos/search?name=${searchTodo}`,
            type: `GET`,
            headers: {
                token: localStorage.token,
                id: localStorage.id
    ...
    $("#navbar__add-member").on('click', function() {
        ...
        $.ajax({
            url: `${BASE_URL}/api/projects/${localStorage.projectId}/addMember`,
            type: `PATCH`,
            data: {
                email
            },
